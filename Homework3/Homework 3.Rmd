...
explication Pruning M1 and M2 techniques = TO DO
...
With the method "first" : TO DO explain "first" method
...
With the method "majority" : TO DO explain "majority" method
...
TO DO : Laplace accuracy ??? definition
...
```{r eval=FALSE, include=FALSE}
classifierFOIL <- FOIL(Result~ ACE.1 + ACE.2 + UFE.1 + UFE.2, data= tennisTrain, max_len = 4, min_gain = 0.01, best_k = 5, disc.method = "ameva", family = "binomial")
classifierFOIL
```
...
classifierFOIL <- FOIL(Species~ ., data= iris, max_len = 3, min_gain = .1, best_k = 5, disc.method = "chi2", family = "binomial")
classifierFOIL
```
```{r eval=FALSE, include=FALSE}
# make predictions
