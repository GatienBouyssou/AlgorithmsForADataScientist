...
classifierRCAR = RCAR(Species ~ ., data= irisTrain, parameter = list(support = 0.01, confidence = 0.3), alpha = 1, disc.method = "chi2")
classifierRCAR
```

```{r}
# make predictions
classifierRCAR.prediction = predict(classifierRCAR, irisTest)
classifierRCAR.confusion_matrix = table(classifierRCAR.prediction, true = irisTest$Species)
...
classifierCMAR <- CMAR(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2,  data= tennisTransactionsTrain)
classifierCMAR
```
