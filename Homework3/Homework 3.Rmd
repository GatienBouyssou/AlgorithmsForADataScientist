...
inspect(tennisTransactions1[assign == 1])
```
...
```{r fig.height=5, fig.width=6, fig.align = "center"}
d <- dissimilarity(tennis_rules,method = "affinity", which = "associations")
clusters_hiearchical <- hclust(d, method = "complete")
plot(clusters_hiearchical)
```
...
classifier <- CBA(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, data = tennisTransactionsTrain, supp = 0.05, conf = 0.4, verbose = TRUE)
```
