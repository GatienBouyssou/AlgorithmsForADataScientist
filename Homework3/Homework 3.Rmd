...
#install_LUCS_KDD_CMAR(force = FALSE,source = "https://cgi.csc.liv.ac.uk/~frans/KDD/Software/CMAR/cmar.tgz")
...
```{r}
classifierRCAR = RCAR(Result ~ ., data= tennisTrain, parameter = list(support = 0.01, confidence = 0.3), alpha = 1, disc.method = "chi2")
classifierRCAR
```
```{r}
# make predictions
classifierRCAR.prediction = predict(classifierRCAR, tennisTest)
classifierRCAR.confusion_matrix = table(classifierRCAR.prediction, true = tennisTest$Result)
...
```{r}
mush_data <-read.csv('./dataPython/mushrooms.csv')
```
...
```{r}
# make predictions
classifierRCAR.prediction = predict(classifierRCAR, )
classifierRCAR.confusion_matrix = table(classifierRCAR.prediction, true = )
classifierRCAR.confusion_matrix
```
...
FOIL (Quinlan and Cameron-Jones, 1995) learns rules and then use them as a classifier following Xiaoxin and Han (2003).
...
classifierFOIL <- FOIL(Species~ ., data= iris, max_len = 3, min_gain = .1, best_k = 5, disc.method = "chi2")
classifierFOIL
```
```{r}
# make predictions
classifierFOIL.prediction = predict(classifierFOIL, irisTest)
classifierFOIL.confusion_matrix = table(classifierFOIL.prediction, true = irisTest$Species)
...
FROM THIS POINT IT DOESN'T WORK AT ALL 
...
### Example on R
```{r}
LUCS_KDD(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, tennisTransactionsTrain, method = "CMAR")
```

```{r eval=FALSE, include=FALSE}
options(java.parameters = "-Xmx1024m")
classifierCMAR <- CMAR(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2,  data= tennisTransactionsTrain, support = 0.1, confidence = 0.5,disc.method = "mdlp", verbose = FALSE)
classifierCMAR
```
...
```{r eval=FALSE, include=FALSE}
classifierCPAR <- CPAR(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, best_k = 5, data= tennisTransactionsTrain)
classifierCPAR
```
