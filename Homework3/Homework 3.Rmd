...
tennisData <- tennisData[,3:7]
tennisData <- data.frame(tennisData)
head(tennisData)
```
...
inspect(head(tennis_rules[assign == 1],5))
```
...
```{r , eval =TRUE, fig.height=5, fig.width=6, fig.align = "center"}
inspect(head(tennis_rules[assign == 2],5))
```
...
```{r}
classifier_rules <- CBA_ruleset(Result ~ .,tennis_rules)
```

```{r}
# make predictions
classifier.prediction = predict(classifier_rules, tennis_rules)
classifier.confusion_matrix = table(classifier.prediction,response(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, tennis_rules))
classifier.confusion_matrix
```
...
```{r}
classifier = RCAR(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, data= tennisTrain, lambda = NULL, alpha = 1, disc.method = "chi2")
classifier
```
...
str(Glass)
response(Type ~ ., Glass)
classifier <- FOIL(Type ~ ., data= Glass, max_len = 3, min_gain = .7, best_k = 5,disc.method = "chi2")
```
...
```{r}
CPAR(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, best_k = 5, data= tennisTransactionsTrain, support = 0.1, confidence = 0.5,disc.method = "mdlp", verbose = FALSE)
```
