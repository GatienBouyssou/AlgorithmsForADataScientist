...
#install_LUCS_KDD_CPAR(force = FALSE,source = "https://cgi.csc.liv.ac.uk/~frans/KDD/Software/FOIL_PRM_CPAR/foilPrmCpar.tgz")
```
...
With the Jaccard measure if A and B are not similar at all, then, Jaccard(A, B) will be equal to 0 because . When $P(A \cap B)$ increase then $P(A \cup B)$ decrease assuming that P(A) and P(B) are constant. So, if $P(A \cap B)$ increase should Jaccard(A, B) as well. The closer Jaccard(A, B) comes to 1 the more similar A and B.

## Loevinger

$Loevinger(A, B) = 1- \frac{P(A \cap \neg{B})}{P(A)P(\neg{B})}$
...
$Laplace(A,B) = \frac{N_{AB}+1}{N_{A}+2}$

With Laplace when the A and B are independent $N_{AB} = 0$ and :

$Laplace(A,B) = \frac{1}{N_{A}+2} \approx 0$ when  $N_{A}$ is big.
...
\item \textbf{Max itemsets} : An itemset X is a maximal frequent itemset (or max-itemset) in set S if X is  frequent,  and  there  exists  no super-itemset Y such that XY and Y is frequent in S.
\item \textbf{Closed itemsets} : An itemset X is a closed frequent itemset in set S if X is  both closed and frequent in S.
\item \textbf{Free or Generator itemsets} : Free itemsets are itemsets that are not included in any closure of their proper sub-set. It means that it has no subset whith the same support.
\item \ textbf{Largest tiles} : It is the biggest are in a dataset. The area is equal to the frequency multiplyed by the size of the dataset.
\end
...
```{python}
# To distribution the distribution of each column
...
CPAR inherits the basic idea of FOIL in rule generation and integrates the features of associative classification in predictive rule analysis. In comparison with associative classification, CPAR has the following advantages: (1) CPAR generates a much smaller set of highquality predictive rules directly from the dataset; (2) to avoid generating redundant rules, CPAR generates each rule by considering the set of \already generated" rules; and (3) when predicting the class label of an example, CPAR uses the best k rules that the example satisfies.
