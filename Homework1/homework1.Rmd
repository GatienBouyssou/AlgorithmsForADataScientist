...
$$ Adusted R^2 = 1 -  \frac{\frac{RSS}{n-p-1}}{\frac{TSS}{n-1}}$$ where : p is the number of predictors in the model and n the size of the data set.
...
$$ C_p = \frac{RSS + 2 p \hat{\sigma}^2}{n}$$ where $\hat{\sigma}$ is the variance estimate of the error.
...
$$ y_{i}= \beta_{0} + \beta_{1}x_{i1} + \beta_{2}x_{i2} + ... + \beta_{p}x_{ip} + \epsilon$$
...
\begin{itemize}
\item Leave out one data point and build the model on the rest of the data set
\item Test the model against the data point that is left out at step 1 and record the test error associated with the prediction 
\item Repeat the process for all data points
\item Compute the overall prediction error by taking the average of all these test error estimates recorded at step 2.
\end{itemize}
