...
There is two types of discriminant Analysis. The first one is the Linear Discriminant Analysis (LDA). To use that model, each class need to be drawn from a multivariate Gaussian distribution and the covariance for each class needs to be equals (the groups of points need to have the same shape).
...
Once the weights are updated we iterate until the amount of errors become acceptable.
