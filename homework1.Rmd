...
deg = 1:25
for (d in deg) {
  modelD <-  randomForest(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, tennisTrain, 
                          mtry = 6, ntree = 500, nodesize=d,importance = TRUE)
...
#The best model is
modelD <-  randomForest(Result ~ ACE.1 + ACE.2 + UFE.1 + UFE.2, tennisTrain, 
                        mtry = 6, ntree = 500, nodesize=which.min(MSE),importance = TRUE)
